...
figure;
bar(xout, nnn);
xlabel('clustering coef c');ylabel('probability p(c)');title('32-node clustering coef distribution');
...
figure;
bar(xout, nn);
xlabel('shortest path length l');ylabel('probability p(l)');title('32-node path length distribution');
...
figure;
bar(xout, n);
xlabel('degree k');ylabel('probability p(k)');title('32-node degree(indegree & outdegree) distribution');